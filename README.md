...
- **再開日→カレンダー作業ブロック**: `schedule_tab` に `calendar` を渡すと、その日のカレンダーへタブ名の作業ブロックを登録する（`calendarCommand` と同じスクリプトの `add` / `free` / `delete-uid`（無ければ `delete`）を使用。`add` が `uid: …` 行を出力すると、以後の削除はその uid で行う。開始時刻省略時は空き時間の先頭、取れなければ 10:00）。`syncCode` を渡すと CrowdLog sync code を notes に入れる。再開日を変えるとブロックも追従し、解除すると削除。自動整理hook（`scripts/setup.sh autoschedule`）は `~/.mimiterm/calendar-sync.json` があればタブ名/グループ名のルールで sync code を決めて同時に登録する（書式は `scripts/auto_schedule.py` 冒頭のコメント参照）
